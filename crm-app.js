...
// >>> FITUR PROMO TANGGAL 1-15 & LAYANAN KONSULTASI <<<
// Setiap service_types punya kolom: is_promo_scheduled, promo_day_start, promo_day_end, promo_price
// (dibuat lewat SQL di Supabase — lihat instruksi terpisah).
//
// Aturannya:
// - Layanan "konsultasi" (harga default_price/promo_price = 0 atau kosong, misalnya
//   Toko Online / Pembuatan Website Custom yang harganya menyesuaikan setelah ngobrol
//   dengan tim) SELALU tampil, tidak terikat jendela promo tanggal.
// - Kalau HARI INI ada di dalam jendela promo salah satu layanan promo -> tampilkan
//   layanan-layanan promo yang jendelanya sedang aktif hari ini (+ layanan konsultasi).
// - Kalau tidak ada jendela promo yang aktif hari ini -> tampilkan semua layanan
//   NON-promo (langganan bulanan dsb) seperti biasa (+ layanan konsultasi).
function servicePrice(s){
  return s.is_promo_scheduled ? s.promo_price : s.default_price;
}
function isConsultationService(s){
  const price = servicePrice(s);
  return price === null || price === undefined || Number(price) === 0;
}
function getVisibleServiceTypes(){
  const today = new Date().getDate(); // tanggal hari ini, 1-31
  const activeServices = SERVICE_TYPES.filter(s =>
    s.active && s.is_promo_scheduled &&
    today >= s.promo_day_start && today <= s.promo_day_end
  );
  const alwaysVisible = SERVICE_TYPES.filter(s => s.active && isConsultationService(s));
  const dedupe = (arr) => Array.from(new Map(arr.map(s=>[s.id,s])).values());
  if(activeServices.length > 0) return dedupe([...activeServices, ...alwaysVisible]);
  const normal = SERVICE_TYPES.filter(s => s.active && !s.is_promo_scheduled && !isConsultationService(s));
  return dedupe([...normal, ...alwaysVisible]);
}
...
async function loadReferenceData(){
  ...
  fService.innerHTML = getVisibleServiceTypes().map(s=>{
    const price = servicePrice(s);
    return `<option value="${s.id}" data-hours="${s.default_hours}">${s.name} — ${fmtRupiah(price)}</option>`;
  }).join('');
...
async function loadOrders(){
  ...
  if(!current){
    currentBox.innerHTML = '<p class="text-dim" id="noCurrent">Tidak ada order yang sedang dikerjakan.</p>';
  } else {
    ...
    currentBox.innerHTML = `
      <div class="ticket current">
        ...
        <div class="ticket-body">
          ...
          ${current.notes ? `<div class="meta" style="white-space:pre-wrap;">📝 Catatan customer: ${current.notes}</div>` : ''}
          ...
          ${paymentProofHTML(current)}
        </div>
        <div class="ticket-actions">
          ${(current.payment_status !== 'paid' && !current.payment_proof) ? `<button class="btn btn-ghost btn-sm" onclick="generateInvoice('${current.id}')">Buat tagihan</button>` : ''}
    ...
    waitBox.innerHTML = waiting.map((o,idx) => `
      <div class="ticket">
        ...
        <div class="ticket-body">
          ...
          ${o.notes ? `<div class="meta" style="white-space:pre-wrap;">📝 Catatan customer: ${o.notes}</div>` : ''}
          ${paymentProofHTML(o)}
        </div>
        <div class="ticket-actions">
...
document.getElementById('addOrderBtn').addEventListener('click', async () => {
  ...
  const customer_email = document.getElementById('f_email').value.trim().toLowerCase();
  ...
  // Email WAJIB: dipakai untuk menghubungkan order ke akun Member Area customer.
  if(!customer_email){ toast('Email customer wajib diisi.'); document.getElementById('f_email').focus(); return; }
  if(!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(customer_email)){ toast('Format email belum benar. Contoh: nama@email.com'); document.getElementById('f_email').focus(); return; }

  const { error } = await sb.from('orders').insert({
    customer_name, contact, customer_email, service_type_id, landing_page_id, estimated_hours, tier_id, status:'waiting'
  });
  ...
  tbody.innerHTML = data.map(o => `
    <tr>
      ...
      <td style="display:flex; flex-wrap:wrap; align-items:center; gap:4px;">
        ${(o.payment_status !== 'paid' && !o.payment_proof) ? `<button class="btn btn-ghost btn-sm" onclick="generateInvoice('${o.id}')">Buat tagihan</button>` : ''}
        ${o.payment_proof ? `<img class="proof-thumb" src="${o.payment_proof}" onclick="window.open('${o.payment_proof}','_blank')" title="${o.payment_sender_name||''} · ${o.payment_sender_bank||''}">` : ''}
        ${o.payment_status==='verifying' ? `<button class="btn btn-primary btn-sm" onclick="verifyManualPayment('${o.id}')">✅ Tandai Lunas</button>` : ''}
...
function fmtRupiah(v){
  ...
  if(Number(v) === 0) return 'Konsultasi';
  return 'Rp' + Number(v).toLocaleString('id-ID');
}
function paymentLabel(s){
  return { unpaid:'Belum ditagih', pending:'Menunggu bayar', verifying:'Menunggu Verifikasi', paid:'Lunas', failed:'Gagal', expired:'Kedaluwarsa' }[s] || s;
}
function paymentBadgeClass(s){
  return { paid:'done', pending:'in_progress', verifying:'in_progress', unpaid:'waiting', failed:'open', expired:'open' }[s] || 'waiting';
}
// Kartu kecil bukti transfer manual (nama pengirim, bank asal, nominal,
// thumbnail bukti yang bisa diklik untuk lihat ukuran penuh) + tombol
// verifikasi -- HANYA muncul kalau order ini memang pakai transfer manual
// (status 'verifying' atau sudah pernah diverifikasi lewat jalur ini,
// ditandai adanya payment_proof). Verifikasi WAJIB dilakukan manual oleh
// admin setelah mencocokkan dengan mutasi rekening asli -- upload bukti
// SAJA tidak pernah otomatis menandai lunas.
function paymentProofHTML(o){
  if(!o.payment_proof) return '';
  return `
    <div class="proof-box">
      <img class="proof-thumb" src="${o.payment_proof}" onclick="window.open('${o.payment_proof}','_blank')" title="Klik untuk lihat ukuran penuh">
      <div class="proof-meta">
        <div><b>${o.payment_sender_name||'-'}</b> · ${o.payment_sender_bank||'-'}</div>
        <div class="text-dim">Nominal diklaim: ${fmtRupiah(o.payment_amount)}</div>
        <div class="text-dim">Dikirim: ${fmtDateTime(o.payment_submitted_at)}</div>
      </div>
      ${o.payment_status==='verifying' ? `<button class="btn btn-primary btn-sm" onclick="verifyManualPayment('${o.id}')">✅ Cocok, Tandai Lunas</button>` : ''}
    </div>`;
}
async function verifyManualPayment(orderId){
  const confirmed = window.confirm('Konfirmasi: Anda sudah mengecek mutasi rekening asli dan nominal + waktu transfer ini memang benar masuk?\n\nJangan tap Ya hanya berdasarkan gambar bukti transfer saja -- cek dulu mutasi rekeningnya.');
  if(!confirmed) return;
  const { error } = await sb.from('orders').update({ payment_status:'paid' }).eq('id', orderId);
  if(error){ toast('Gagal memperbarui status: ' + error.message); return; }
...
const PRODUCT_LABELS = {
  ...
  'menu-resto-qr': 'Menu Resto QR (Kalasan Mas Fery & sejenis)'
};
...
// >>> PERBAIKAN <<<
// Elemen 'activateSubBtn' (dan form ns_product/ns_customer_id/ns_customer_name)
// belum ada di index.html saat ini. Sebelumnya baris ini langsung memanggil
// .addEventListener pada null, yang membuat SELURUH sisa script berhenti
// dieksekusi (termasuk tombol +Tambah di tab Pengaturan & proses buka
// dashboard). Sekarang dicek dulu supaya tidak mematikan script lain.
const activateSubBtnEl = document.getElementById('activateSubBtn');
if(activateSubBtnEl){
  activateSubBtnEl.addEventListener('click', async () => {
...
async function loadChatThreads(){
  ...
  const chatListEl = document.getElementById('chatThreadList');
  if(error){ if(chatListEl) chatListEl.innerHTML = `<div class="empty-row" style="padding:16px;">Gagal memuat: ${error.message}</div>`; return; }
...
function renderChatThreadList(){
  ...
  if(!list) return; // >>> PERBAIKAN: elemen ini belum ada di index.html saat ini, cegah crash
  ...
  if(countEl) countEl.textContent = `(${CHAT_THREADS.length} percakapan${totalUnread ? ', '+totalUnread+' belum dibaca' : ''})`;
...
function renderChatDetail(thread){
  ...
  if(!box) return; // >>> PERBAIKAN: elemen ini belum ada di index.html saat ini, cegah crash
...
function updateChatNavBadge(){
  ...
  if(!badge) return; // >>> PERBAIKAN: elemen ini belum ada di index.html saat ini, cegah crash
